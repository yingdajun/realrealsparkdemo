...
//    val Array(actionInPath,output,mode) = args
    ...
    // 处理曝光、点击日志数据，解析有效数据
    val actionLog = sc.textFile("D:\\sparkdata\\bigdata\\2rd_data\\ch09\\action.txt").map{
      case line => {
    ...
    // 保存解析好的action日志
    actionLog.saveAsTextFile("./output/ch09"+"/actionLog")
    // 过滤异常点击，只有当天曝光的点击才有效
    val validActionLog = clickFilter(actionLog)

    //日活跃统计、异常计算
    dayStat(validActionLog,"./output/ch09"+"/dayActive")
    sc.stop()
  }
